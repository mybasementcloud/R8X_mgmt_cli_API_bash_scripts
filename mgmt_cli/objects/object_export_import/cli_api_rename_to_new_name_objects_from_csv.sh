...
# =================================================================================================
# -------------------------------------------------------------------------------------------------
# START : Main Operational repeated proceedures - RenameToNewNameWithSetUpdateSimpleObjects - Rename
# -------------------------------------------------------------------------------------------------
...
RenameToNewNameWithSetUpdateSimpleObjects () {
    ...
    echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
    echo `${dtzs}`${dtzsep} 'RenameToNewNameWithSetUpdateSimpleObjects : Done with Rename to new name '${APICLIcomplexobjectstype}' '${APICLICSVobjecttype}' ( '${APICLIexportnameaddon}' ) using CSV File : '${APICLIImportCSVfile} | tee -a -i ${logfilepath}
    echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
    ...
    if [ ${errorreturn} != 0 ] ; then
        ...
        if ${ABORTONERROR} ; then
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} 'Error { '${errorreturn}' } in RenameToNewNameWithSetUpdateSimpleObjects procedure' | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            ...
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} 'Error { '${errorreturn}' } in RenameToNewNameWithSetUpdateSimpleObjects procedure, but continueing' | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            #return ${errorreturn}
        fi
    fi
...
ConfigureSetComplexObjects () {
    ...
    if [ $(expr ${APIobjectminversion} '<=' ${CurrentAPIVersion}) -eq 1 ] ; then
        ...
        echo `${dtzs}`${dtzsep} 'Renaming via Set operation '${APICLIcomplexobjectstype}' '${APICLICSVobjecttype}' from CSV File : "'${APICLIImportCSVfile}'"' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} '  mgmt_cli parameters : "'${MgmtCLI_Set_OpParms}'"' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} '  and dump to "'${OutputPath}'"' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        ...
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'ConfigureSetComplexObjects: Done with Renaming via Set operation '${APICLIcomplexobjectstype}' '${APICLICSVobjecttype}' ( '${APICLIexportnameaddon}' ) using CSV File : '${APICLIImportCSVfile} | tee -a -i ${logfilepath}
