...
export expected_script_work_dir=object_export_import
export expected_script_work_folder=${customermgmtclipathroot}/objects/object_export_import

echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
echo `${cexdtzs}`${cexdtzsep} 'Current APIScriptActionFileRoot '"${APIScriptActionFileRoot}" | tee -a -i ${logfilepath}
echo `${cexdtzs}`${cexdtzsep} 'Current expected_script_work_dir '"${expected_script_work_dir}" | tee -a -i ${logfilepath}
echo `${cexdtzs}`${cexdtzsep} 'Current expected_script_work_folder '"${expected_script_work_folder}" | tee -a -i ${logfilepath}
echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}

if [ -r "${APIScriptJSONActionFilename}" ] ; then
    # found the script in the local directory, use that
    #export test_script_work_folder=.
    echo `${cexdtzs}`${cexdtzsep} 'Current folder '`pwd` | tee -a -i ${logfilepath}
    export APIScriptActionFileRoot=`pwd`
elif [ -r "./${expected_script_work_dir}/${APIScriptJSONActionFilename}" ] ; then
    # found the script in the local directory, use that
    #export APIScriptActionFileRoot=.
    echo `${cexdtzs}`${cexdtzsep} 'Current folder '`pwd` | tee -a -i ${logfilepath}
    pushd "./${expected_script_work_dir}" >> ${logfilepath}
    errorreturn=$?
    
    if [ ${errorreturn} -ne 0 ] ; then
        # we apparently didn't start where expected, so dumping
        echo `${cexdtzs}`${cexdtzsep} 'Required target folder '"./${expected_script_work_dir}"' not found, exiting!' | tee -a -i ${logfilepath}
        #popd >> ${logfilepath}
        exit 254
    else
        #OK, so we are where we want to be relative to the script targets
    ...
    # Return to the script operations folder
    popd >> ${logfilepath}
elif [ -r "${expected_script_work_folder}/${APIScriptJSONActionFilename}" ] ; then
    # found the script in the local directory, use that
    #export APIScriptActionFileRoot=.
    echo `${cexdtzs}`${cexdtzsep} 'Current folder '`pwd` | tee -a -i ${logfilepath}
    pushd "${expected_script_work_folder}" >> ${logfilepath}
    errorreturn=$?
    
    if [ ${errorreturn} -ne 0 ] ; then
        # we apparently didn't start where expected, so dumping
        echo `${cexdtzs}`${cexdtzsep} 'Required target folder '"${expected_script_work_folder}"' not found, exiting!' | tee -a -i ${logfilepath}
        #popd >> ${logfilepath}
        exit 254
    else
        #OK, so we are where we want to be relative to the script targets
    ...
    echo `${cexdtzs}`${cexdtzsep} 'Current folder '`pwd` | tee -a -i ${logfilepath}
    pushd ${APIScriptActionFileRoot} >> ${logfilepath}
    errorreturn=$?
    
    if [ ${errorreturn} -ne 0 ] ; then
        # we apparently didn't start where expected, so dumping
        echo `${cexdtzs}`${cexdtzsep} 'Required target folder '"${APIScriptActionFileRoot}"' not found, exiting!' | tee -a -i ${logfilepath}
        #popd >> ${logfilepath}
        exit 254
    else
        #OK, so we are where we want to be relative to the script targets
        if [ -r "${APIScriptJSONActionFilename}" ] ; then
            ...
            echo `${cexdtzs}`${cexdtzsep} 'Required target file '"${APIScriptActionFileRoot}/${APIScriptJSONActionFilename}"' not found, exiting!' | tee -a -i ${logfilepath}
            #popd >> ${logfilepath}
            exit 254
        fi
    fi
...
echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
echo `${cexdtzs}`${cexdtzsep} 'Final APIScriptActionFileRoot '"${APIScriptActionFileRoot}" | tee -a -i ${logfilepath}
echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
...
# =================================================================================================
# -------------------------------------------------------------------------------------------------
# START : Main Operational repeated proceedures - ImportSimpleObjects
# -------------------------------------------------------------------------------------------------
...
ImportSimpleObjects () {
    ...
    if [ ${errorreturn} != 0 ] ; then
        ...
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'Discard '${APICLIobjecttype}' object changes due to error to allow for continue!  This could take a while...' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        
        . ${mgmt_cli_API_operations_handler} DISCARD "$@"
        discarderrorreturn=$?
        
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'Discard operation Error Return Code = '${discarderrorreturn} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        
        if [ ${discarderrorreturn} != 0 ] ; then
            errorreturn=${discarderrorreturn}
        else
            errorreturn=0
        fi
    ...
    echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
    echo `${dtzs}`${dtzsep} 'ImportSimpleObjects : Done with Importing '${APICLIobjecttype}' ( '${APICLIexportnameaddon}' ) using CSV File : '${APICLIImportCSVfile} | tee -a -i ${logfilepath}
    echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
    ...
    if [ ${errorreturn} != 0 ] ; then
        ...
        if ${ABORTONERROR} ; then
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} 'Error { '${errorreturn}' } in ImportSimpleObjects procedure' | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            ...
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} 'Error { '${errorreturn}' } in ImportSimpleObjects procedure, but continueing' | tee -a -i ${logfilepath}
            echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
            #return ${errorreturn}
        fi
    fi
...
CheckAPIVersionAndExecuteOperation () {
    ...
    echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
    echo `${dtzs}`${dtzsep} 'Required minimum API version for object : '${APICLIobjectstype}' ( '${APICLIexportnameaddon}' ) is API version = '${APIobjectminversion} | tee -a -i ${logfilepath}
...
ConfigureSetComplexObjects () {
    ...
    case "${script_main_operation}" in
        ...
        'set-update' )
            if ! ${APIobjectdoupdate} ; then
                echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
                echo `${dtzs}`${dtzsep} 'Object '${APICLIcomplexobjectstype}' does NOT support SET [SET-UPDATE]!  APIobjectdoexport = '${APIobjectdoexport} | tee -a -i ${logfilepath}
                echo `${dtzs}`${dtzsep} 'Skipping!' | tee -a -i ${logfilepath}
                echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
                echo `${dtzs}`${dtzsep} '-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-' | tee -a -i ${logfilepath}
    ...
    if [ $(expr ${APIobjectminversion} '<=' ${CurrentAPIVersion}) -eq 1 ] ; then
        ...
        echo `${dtzs}`${dtzsep} 'Importing via Set operation '${APICLIcomplexobjectstype}' '${APICLICSVobjecttype}' from CSV File : "'${APICLIImportCSVfile}'"' | tee -a -i ${logfilepath}
        ...
        if [ ${errorreturn} != 0 ] ; then
            # Something went wrong, terminate
            echo `${dtzs}`${dtzsep} 'ConfigureSetComplexObjects : Problem during publish operation! error return = { '${errorreturn}' }' | tee -a -i ${logfilepath}
        ...
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'ConfigureSetComplexObjects : Done with Importing via Set operation '${APICLIcomplexobjectstype}' '${APICLICSVobjecttype}' ( '${APICLIexportnameaddon}' ) using CSV File : '${APICLIImportCSVfile} | tee -a -i ${logfilepath}
...
ConfigureImportComplexObjects () {
    ...
    if [ $(expr ${APIobjectminversion} '<=' ${CurrentAPIVersion}) -eq 1 ] ; then
        ...
        echo `${dtzs}`${dtzsep} 'Importing via Add operation '${APICLIcomplexobjectstype}' '${APICLICSVobjecttype}' from CSV File : "'${APICLIImportCSVfile}'"' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} '  mgmt_cli parameters : "'${MgmtCLI_Set_OpParms}'"' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} '  and dump to "'${OutputPath}'"' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        ...
        if [ ${errorreturn} != 0 ] ; then
            # Something went wrong, terminate
            echo `${dtzs}`${dtzsep} 'ConfigureImportComplexObjects : Problem during publish operation! error return = { '${errorreturn}' }' | tee -a -i ${logfilepath}
        ...
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'ConfigureImportComplexObjects : Done with Importing via Add operation '${APICLIcomplexobjectstype}' '${APICLICSVobjecttype}' ( '${APICLIexportnameaddon}' ) using CSV File : '${APICLIImportCSVfile} | tee -a -i ${logfilepath}
